...
function create(namespace, contract, methodsArr, eventsArr) {
  ...
  return () => {
    const methods = methodsArr.reduce((reduction, method) => {
      ...
      return [
        ...
        takeEvery(SEND_TYPES.SEND, function* watchTransactionChannel({
          ...
          if (!options.from) {
            const account = yield select(web3Selectors.accounts.selectAccounts);
    ...
    const events = eventsArr.reduce((reduction, event) => {
